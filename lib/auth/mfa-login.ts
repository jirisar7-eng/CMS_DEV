...
export async function recordFailure(
  ...
  const execute = async (client: Prisma.TransactionClient) => {
    await recordMfaFailure({ accountHash, tx: client });
    await recordFailedMfaChallengeAttempt(token, userId, client);
  };
...
async function verifyMfaLoginInTransaction(
  ...
  const fail = async () => {
    await recordFailure(challengeToken, userId, accountHash, tx);
    state.failureRecorded = true;
    return null;
  };

  const challenge = challengeToken
    ? await tx.mfaChallenge.findUnique({
        where: { tokenHash: hashMfaChallengeToken(challengeToken) },
        select: { userId: true },
      })
    : null;
  if (!challenge) return await fail();
  ...
  if (
    ...
    user.status !== "ACTIVE" ||
    mfa?.status !== "ENABLED" ||
    !(await checkMfaAllowed({ accountHash, tx }))
  ) {
    return await fail();
  }
  ...
  if (/^\d{6}$/.test(code) && mfa.totpSecretEncrypted) {
    try {
      verified = verifyTotpToken(decryptSecret(mfa.totpSecretEncrypted), code);
    } catch {
      verified = false;
    }
  } else if (/^(?:[A-Fa-f0-9]{20}|[A-Fa-f0-9]{5}(?:-[A-Fa-f0-9]{5}){3})$/.test(code)) {
    const codes = await tx.mfaRecoveryCode.findMany({
  ...
  if (!(await consumeMfaChallenge(challengeToken, userId, tx))) return await fail();

  if (recoveryId) {
    const used = await tx.mfaRecoveryCode.updateMany({
      where: { id: recoveryId, mfaId: mfa.id, usedAt: null },
...
export async function verifyMfaLogin(token: string | null, input: unknown): Promise<string | null> {
  const state: MfaTransactionState = {
    ...
    userId: "",
    accountHash: computePrivacyIdentifier("account", ""),
    failureRecorded: false,
    verified: false,
  };
  try {
    return await prisma.$transaction((tx) => verifyMfaLoginInTransaction(token, input, tx, state), {
      timeout: 15000,
    });
  } catch {
    if (!state.failureRecorded && !state.verified) {
      try {
        await recordFailure(state.token, state.userId, state.accountHash);
        state.failureRecorded = true;
      } catch (fallbackError) {
        console.error("[MfaLogin] Fallback failure accounting failed:", fallbackError);
      }
    }
    return null;
  }
