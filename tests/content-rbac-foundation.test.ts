...
test('Content RBAC Foundation - 6. Existing projectId-aware RBAC logic remains present', () => {
  ...
  // Check user permission overrides check specific projectId first
  assert.match(
    rbacContent,
  ...
  // Check specific override prioritized over global
  assert.match(
    rbacContent,
    /const\s+specificOverride\s*=\s*overrides\.find/,
    'Must find specific project override'
  );
  assert.match(
    rbacContent,
    /const\s+globalOverride\s*=\s*overrides\.find/,
    'Must find global override'
  );

  // Check explicit DENY overrides everything
  assert.match(
    rbacContent,
    /if\s*\(!specificOverride\.isGranted\)\s*return\s+false;/,
    'Specific DENY must return false'
  );
  assert.match(
    rbacContent,
    /if\s*\(!globalOverride\.isGranted\)\s*return\s+false;/,
    'Global DENY must return false'
  );
});
