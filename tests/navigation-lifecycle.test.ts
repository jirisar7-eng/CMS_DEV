...
describe('SYN-NAV-002 Phase A: Navigation Published Snapshot Lifecycle', () => {
  ...
  it('9. parses and validates valid snapshot JSON, and rejects malformed or invalid inputs', () => {
    const validSnapshot = {
      key: 'header-main',
      name: 'Hlavní',
      context: 'HEADER',
    ...
    assert.strictEqual(parsePublishedNavigationSnapshot({
      ...validSnapshot,
      items: [{ id: 'x', label: 'x', type: 'EXTERNAL_LINK', externalUrl: 'javascript:evil()' }]
    }), null);
  });
  ...
  it('11. verifies publishedVersion <= version contract', () => {
