  ...
  try {
    ...
    if (action[0] === 'items' && action.length === 2) {
      ...
      if (body.externalUrl !== undefined) {
        ...
        dataToUpdate.externalUrl = body.externalUrl;
      }
      ...
      if (body.anchor !== undefined) dataToUpdate.anchor = body.anchor;
...
export async function DELETE(
  ...
  try {
    ...
    if (action?.[0] === 'items' && action.length === 2) {
      const itemId = action[1];
      // Due to CASCADE, deleting the item will delete its children automatically
